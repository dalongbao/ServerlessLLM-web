...
import { useChatSelector } from "./useChatSelector"; import { Chat } from "@/context/types";
...
const ChatCarousel = ({ hook, animateTo, renamingChatId, setRenamingChatId, setDeletingChat }: { hook: ReturnType<typeof useChatSelector>; animateTo: (target: number) => void; renamingChatId: string | null; setRenamingChatId: (id: string | null) => void; setDeletingChat: (chat: Chat) => void; }) => {
      ...
      {OFFSETS.map((o) => {
        ...
        return (
          <div
            ...
            onKeyDown={(e) => (e.key === 'Enter' || e.key === ' ') && animateTo(idx)}
          ...
          >
            ...
            <div className={`relative w-full h-full rounded-lg bg-gray-50 flex flex-col justify-center items-center p-2 z-20 ${slideClass} ${isCentre ? "shadow-md" : "shadow-sm"}`}>
      ...
      {chats.map((chat, idx) => {
        ...
        return (
          <div
            ...
            onAnimationEnd={() => { if (isNew) setNewChatId(null); }}
            style={{ height: `${CARD_H}px` }}
            className={`group relative w-[90%] flex-shrink-0 transition-all duration-200 cursor-pointer focus:outline-none ${idx > 0 && "mt-2"} ${isNew && "animate-wiggle-once"}`}
          >
  ...
  return (
    <>
      ...
      <aside ref={containerRef} tabIndex={0} className={`${baseClasses} ${layoutClasses}`} onWheel={onWheel} onKeyDown={onKeyDown}>
        {isCarousel ? (
          <ChatCarousel hook={hook} animateTo={animateTo} renamingChatId={renamingChatId} setRenamingChatId={setRenamingChatId} setDeletingChat={setDeletingChat} />
        ) : (
